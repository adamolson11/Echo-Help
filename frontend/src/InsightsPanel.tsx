...
export default function InsightsPanel() {
  ...
  async function fetchPatternRadar(days: number) {
    ...
    try {
      ...
      setError(err?.message ?? "Failed to load ticket signals");
    }
  }
  ...
  if (error) {
    return (
      <div className="mt-6 rounded-xl border border-red-700 bg-red-950/60 p-4 text-sm text-red-200">
        Failed to load signal summaries: {error}
      </div>
    );
  }
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-[1.2fr,2fr]">
        ...
        <div className="rounded-xl border border-slate-700 bg-slate-900/60 p-4">
          ...
          <div className="mt-4">
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto pr-1 text-xs text-slate-300">
          ...
          {/* Ticket signal quick stats */}
          <div className="mt-4 border-t border-slate-800 pt-3">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">
                Ticket Signal Snapshot
              </h3>
              <select
            ...
            {!patternRadar && (
              <div className="text-xs text-slate-400">Loading ticket signals…</div>
            )}
              ...
              <div className="space-y-3 text-xs text-slate-300">
                <div className="flex flex-col gap-0.5 text-[11px] text-slate-400">
                  <span className="font-semibold text-slate-200">
                    Ticket Signal Snapshot {patternRadar.meta?.version ? `(v${patternRadar.meta.version})` : ""}
                  </span>
                  <span>
                  ...
                  {patternRadar.stats.last_ticket_at && (
                    <span>
                      Latest ticket signal: {new Date(patternRadar.stats.last_ticket_at).toLocaleString()}
                    </span>
                  )}
                </div>
                ...
                <div className="rounded-md border border-slate-800 bg-slate-950/60 p-2">
                  ...
                  {patternRadar.top_keywords.length === 0 ? (
                    <div className="text-[11px] text-slate-500">
                      No clear queue signals detected in this window.
                    </div>
                  ) : (
                ...
                {patternRadar.top_keywords.length === 0 &&
                  patternRadar.frequent_titles.length === 0 && (
                    <div className="text-[11px] text-slate-500">
                      No meaningful signals detected in this window yet.
                    </div>
                  )}
              </div>
            )}
        ...
        <div className="rounded-xl border border-slate-700 bg-slate-900/60 p-4">
          <h2 className="mb-1 text-sm font-semibold uppercase tracking-wide text-slate-400">
            Feedback Signals {feedbackPatterns?.meta?.version ? `(v${feedbackPatterns.meta.version})` : ""}
          </h2>
          {feedbackPatterns ? (
            ...
            <div className="text-xs text-slate-400">Loading feedback signals…</div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-slate-700 bg-slate-900/60 p-4">
          <h2 className="mb-2 text-sm font-semibold uppercase tracking-wide text-slate-400">
            Reusable Fix Signals
          </h2>
          {/* Existing snippet-based radar now focuses on reusable fix groupings only */}
          {clusters.length === 0 ? (
            <div className="text-sm text-slate-400">
              Not enough outcome notes yet to group reusable fixes. As more techs add notes,
              you&apos;ll start seeing common resolution themes here.
            </div>
          ) : (
